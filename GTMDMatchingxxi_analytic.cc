...
double C1ggR(double const &y, double const &kappa)
{
  const double ky = kappa * y;
  const double ky2 = pow(ky, 2);
  if (kappa < 1) return (y <= 1 ? 8 * apfel::CA * pow(kappa, 2) * y * (1 - y) / pow(1 - ky2, 2) : 0);
  else
    return (y <= 1 ? apfel::CA * (1 + kappa * (y + kappa * (-5 + 3 * ky))) / 2 / kappa / pow(1 + ky, 2)
                   : apfel::CA * (1 - kappa) * (1 + kappa - (1 - 7 * kappa) * ky2) / kappa / pow(1 - ky2, 2));
}
double C1ggL() { return -apfel::CA * apfel::zeta2; }
double C1ggSPV(double const &y, double const &kappa)
{
  if (kappa > 1 && y < 1) return apfel::CA * (1 + 3 * pow(kappa, 2)) / (1 - kappa * y) / 2 / kappa;
  else return 0;
}
...
int main(int argc, char **argv)
{
  ...
  // Get GTMD objects
  const auto GtmdObj = apfel::InitializeGtmdObjectsEvenUU(g, Thresholds, xi);
  ...
  // Sudakov form factor
  const std::function<double(double const &, double const &, double const &, double const &)> Sf = GluonEvolutionFactor(GtmdObj, as, ipt);
  ...
  for (double x = xmin; x <= xmax * (1 + 1e-5); x *= xstp)
    {
      ...
      const std::function<double(double const &)> xGbAn = [=](double const &bT) -> double
      {
        ...
        const std::function<double(double const &)> f = [=](double const &y) -> double
        {
          ...
          return C1gq(y, kappa) * qrk + C1ggR(y, kappa) * glu + C1ggSPV(y, kappa) * (glu - gluxi * (1 + (ky > 1 ? (1 - ky) / ky : 0)));
        };
        const apfel::Integrator I{f};
        const double coup = (ipt > 1 ? as(mub) / apfel::FourPi : 0);
        const double mGPD = gpsd.at(0).Evaluate(x) + coup * (I.integrate(x, 1.1, apfel::eps7) + I.integrate(1.1, x * 1e7, apfel::eps7) +
                                                             C1ggL() * gpsd.at(0).Evaluate(x) + C1ggLPV(x, kappa) * gluxi);
        return bT * Sf(x, b, mu, mu * mu) * mGPD *
               NPFunc->Evaluate(x, bT, mu * mu, 0); // modified! First argument of Sf in APFEL is now x, inside the function kappa is constructed
      };

      const double nm = QCDEvToPhys(F.GetObjects()).at(0).Evaluate(x);
      const double an = DEObj.transform(xGbAn, kT);
      std::cout << x << "  " << nm << "  " << an << "  " << nm / an << "  " << std::endl;
    }
