...
/*
 * This code computes F11 as a function of kT at fixed x, t, and Q for
...
int main(int argc, char** argv)
{
  ...
  apfel::TabulateObject<apfel::Set<apfel::Distribution>> TabulatedPDFs{*(BuildDglap(InitializeDglapObjectsQCD(g, Thresholds), InPDFs, sqrt(pGPDModel->getMuF2Ref()), 0, as)), 100, 1, 1000, 3};
  ...
  const std::function<double(double const&)> xTb = [=] (double const& bT) -> double { return bT * QCDEvToPhys(EvTMDs(bs(bT, mu), mu, mu * mu).GetObjects()).at(ifl).Evaluate(xb) * NPFunc->Evaluate(xb, bT, mu * mu, 0); };
  const apfel::TabulateObject<double> TabxTb{xTb, 100, 0.00005, 100, 5, {}, [] (double const& x)->double{ return log(x); }, [] (double const& x)->double{ return exp(x); }};
  ...
  std::cout << "# kT [GeV]        TMD          GTMD ..." << std::endl;
  for (double qT = qTmin; qT <= qTmax * ( 1 + 1e-5 ); qT += qTstp)
    {
      std::cout << qT << "  " << DEObj.transform(txTb, qT) << "  ";
